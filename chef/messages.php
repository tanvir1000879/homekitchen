                        ...
                        <div class="card-body card-body-inbox overflow-auto">
                          <div class="list-group list-group-flush">
                            ...
                            <?php
                              ...
                              $sql = "SELECT * FROM conversations WHERE chef_id = '$id' ORDER BY id DESC";
                        ...
                        <?php }
                              elseif ($result->num_rows > 0) {
                                foreach ($result as $row) { ?>
                                  <a href="set_receiver.php?receiver_id=<?=$row['user_id']?>" class="list-group-item list-group-item-action d-flex align-items-center">
                                    <div class="text-truncate">
                                      <span class=""><?=$row['last_text']?></span>
                                      <?php
                                        $temp_receiver = $row['user_id'];
                                        $sql2 = "SELECT * FROM users WHERE user_id='$temp_receiver'";
                                        $result2 = $conn->query($sql2);
                                        while($row2 = $result2->fetch_assoc()) { ?>
                                      <div class="small text-gray-500"><b><?=$row2['user_name']?></b> · <?=$row['time_stamp']?></div>
                                    <?php } ?>
                                    </div> 
                                  </a>
                        <?php   }
